...
test("Tencent batch realtime quotes are parsed", () => {
  const symbol = { code: "600519", name: "Kweichow Moutai", market: "aShare", quoteID: "1.600519" };
...
test("English defaults contain no Chinese UI text", () => {
  const state = sanitizeState();
  assert.equal(state.symbols.length, 3);
  for (const symbol of state.symbols) {
    assert.doesNotMatch(symbol.name, /[\u3400-\u9fff]/u);
  }
});
